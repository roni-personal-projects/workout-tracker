    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-[var(--bg-surface)] border border-[var(--bg-border)] rounded p-3 shadow-lg">
          ...
          <p className="text-[var(--chart-4)] font-bold text-lg m-0">
  ...
  return (
    <div className="iron-card">
      <div className="flex flex-col gap-2 mb-6">
        <h3 className="font-bold uppercase tracking-wider m-0">Exercise Frequency</h3>
        <select 
          value={selectedExercise} 
          onChange={e => setSelectedExercise(e.target.value)}
          className="bg-[var(--bg-base)] border border-[var(--bg-border)] rounded p-1 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] max-w-full truncate"
        >
      ...
      {loading ? (
        <div className="h-[250px] flex items-center justify-center text-[var(--text-secondary)] font-mono text-sm animate-pulse">
          Loading data...
        </div>
      ) : (
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
              ...
              <Bar dataKey="value" fill="url(#colorSets)" radius={[4, 4, 0, 0]}>
                {data.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.value > 0 ? "url(#colorSets)" : "transparent"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
